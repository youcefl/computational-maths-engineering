...
Clearly the number of digits in $\overline{i}_{b} = 1 + floor(\dfrac{log(i)}{log(b)})$
is constant as long as i does not cross a power of b so that for b^k <= n < b^(k + 1) we have

$$
    Sm_b(n) = Sm_b(b^k - 1)*b^((k + 1)*(n - b^k + 1)) + sum(i=b^k, n, i*b^((k + 1)*(n - i)))      (1)
$$

Let $c_k = Sm_b(b^k - 1)$ and $s(k, n) = sum(i=b^k, n, i*b^((k + 1)*(n - i)))$
